...
@Injectable()
export class VacantesPlantillaService {
  constructor(private readonly prisma: PrismaService) {}

  create(createVacantesPlantillaDto: CreateVacantesPlantillaDto) {
    return 'This action adds a new vacantesPlantilla';
  }
  ...
  findOne(id: string) {
    return `This action returns a #${id} vacantesPlantilla`;
  }

  update(id: string, updateVacantesPlantillaDto: UpdateVacantesPlantillaDto) {
    return `This action updates a #${id} vacantesPlantilla`;
  }

  remove(id: string) {
    return `This action removes a #${id} vacantesPlantilla`;
  }

  async generatePDFreporteVacantesPlantillas(cont: any): Promise<Buffer> {
    const rep = cont;
    console.log(cont);
    const pdfBuffer: Buffer = await new Promise((resolve) => {
      ...
      this.generateHeader(doc, 'Puestos de Trabajo Vacantes en Plantilla');
      doc.moveDown();
  ...
  generateTable(doc, info) {
    ...
    for (let i = 0; i < info.length; i++) {
      ...
      totalArea.nombre_categocup = totalArea.nombre_categocup + item.categocup;
      totalArea.codigo = totalArea.codigo + item.codigo;
      totalArea.nombre_grupoescala =
        totalArea.nombre_grupoescala + item.grupoescala;
